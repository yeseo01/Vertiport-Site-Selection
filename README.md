...
- Implemented weighted K-Means from scratch with NumPy for vertiport site selection.
- Applied the clustering workflow to 766 archived processed candidate locations to select 17 representative sites.
- Used project-specific weighting signals based on traffic congestion and inter-regional commuter demand.
- Mapped each converged cluster centroid to the nearest valid candidate site within its assigned cluster.


## Reproduced Results

The current repository reproduces the clustering-based site-selection result for the assignment-specified `K = 17`:

| Result | Value |
| --- | ---: |
| Processed candidate locations | 766 |
| Representative vertiport sites | 17 |
| Weighted inertia | 63.535804 |
| Silhouette score | 0.483956 |
...
I also contributed to the weighting strategy, including proposing the use of traffic congestion and inter-regional commuter demand as input signals. Broader preprocessing and site-selection decisions were developed collaboratively by the team.
...
With `random_state=10`, the current implementation reproduces the representative sites generated by the original project implementation for `K = 17`.
